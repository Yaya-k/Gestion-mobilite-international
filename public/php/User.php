...
class User
{
    ...
    public function requestUserData($email,$bdd){
        $req = $bdd->prepare('SELECT id, password,nom, prenom, email,promo FROM user WHERE email = :email');
        $req->execute(array(
            'email' => $email));
        return $req->fetch();
    }
    ...
    public function isertNewMobilite($bdd,$ville, $pays, $date_debut, $date_fin,$id_personne,$statue){
        $sql = "INSERT   demande_mobilite (ville, pays, date_debut, date_fin, id_personne,statue) VALUES(?,?,?,?,?,?)";
        $stmtinsert = $bdd->prepare($sql);
        $result = $stmtinsert->execute([$ville, $pays, $date_debut, $date_fin,$id_personne,$statue]);
